...
files = glob.glob(os.getcwd() + "/en_tn-master/en_tn/*.tsv")
word_count = {}
for fl in files:
  bookname = fl.split("/")[-1].split(".")[0]
  filename = fl.split("/")[-1]
  path = glob.glob(os.getcwd() + "/en_tn-master/en_tn/" + str(filename))
...
# table.columns[0].width = Cm(4) # Try to set column 0 width to 1.0
